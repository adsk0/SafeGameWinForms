...
namespace SafeGameWinForms
{
    public partial class Form1 : Form
    {
        ...
        private int difficaltyLevel = Level.hard;                    // default field is level hard

        public static bool[,] FillingMatrix(bool[,] addMatrix)
        {
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 5; j++)
        ...
        public void BntDifficaltyLevelOnClick(object sender, EventArgs e)  // catches  clicks on difficalty level buttons
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    ...
                    {
                        if (difficaltyLevel == Level.medium && (i > 2 || j > 2))
                        {
                            arrButtons[i, j].Visible = false;
                        }
                    }
        ...
        private static void ShakeMatrix(bool[,] addMatrix)  // shakes matrix
        {
            ...
            for (int i = 0; i < rnd.Next(10, 715); i++)
            {
